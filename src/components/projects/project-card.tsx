      ...
      <div className="p-4 md:p-5">
        <div className="flex justify-between mb-2">
          ...
          {project.is_live_demo && (
            <Badge variant="secondary" className="bg-accent/10 text-accent border-accent/20 text-xs md:text-sm">
              Live Demo
            </Badge>
          )}
        </div>
        ...
        <div className="mt-4 flex justify-between items-center">
          <div className="flex gap-2 flex-wrap">
            {typeof project.technologies === 'string'
              ? project.technologies.split(',').slice(0, 2).map((tech, index) => (
                  <span key={index} className="inline-block text-xs text-muted-foreground px-2 py-1 bg-muted/30 rounded-full">
                    {tech.trim()}
                  </span>
                ))
              : Array.isArray(project.technologies) && project.technologies.slice(0, 2).map((tech, index) => (
                  <span key={index} className="inline-block text-xs text-muted-foreground px-2 py-1 bg-muted/30 rounded-full">
                    {tech}
                  </span>
                ))
          ...
          <Link href={`/projects/${project.id}`} className="text-primary hover:text-primary/80 text-xs md:text-sm font-medium inline-flex items-center transition-colors">
            Details
